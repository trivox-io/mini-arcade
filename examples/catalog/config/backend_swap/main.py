...
DEFAULT_DISCOVER_PACKAGES = [
    "examples.catalog.config.backend_swap",
...
def build_example(**kwargs) -> ExampleSpec:
    ...
    if not isinstance(postfx_defaults, dict):
        postfx_defaults = {}
    postfx_enabled = bool(postfx_defaults.get("enabled", False))
    postfx_active = _list_str(postfx_defaults.get("active", []))
